...
@Component({
  selector: 'app-solicitaracessopjclient',
  templateUrl: './solicitaracessopjclient.component.html',
  styleUrls: ['./solicitaracessopjclient.component.scss']
})
export class SolicitaracessoPJClientComponent implements OnInit {
  ...
  getSeguimentos() {
    this.bemvindoService.pegaSeguimentos().subscribe(res => {
      this.listaSegmentos = [];
      for (var i = 0; i < res.lista.length; i++) {
        this.listaSegmentos.push({
          id: res.lista[i].id,
          nome: res.lista[i].nome,
    ...
    this.bemvindoService.meuCadastro().subscribe(
      (res) => {
        if (res.user) {
          ...
          this.formulario.controls["apelido"].setValue(res.user.nomeempresa);
          this.formulario.controls["nome"].setValue(res.user.nomeempresa);
  ...
  validastep1() {
    if (!this.formulario.controls.nome.valid) {
      ...
      return false;
    } else if (!this.formulario.controls.segmento.valid) {
      this.modalService.showAlert("Informe um Segmento");
      return false;
    }
    return true;
  }
